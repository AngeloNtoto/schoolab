  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (fileName.endsWith('.docx')) {
        ...
        toast.error('Format non supporté (.docx uniquement)');
        setLoading(false);
        return;
      }
                ...
                {importStep === 'initial' && (
                    <div key="initial" className="space-y-6 animate-in fade-in duration-300">
                        ...
                        >
                            ...
                            <p className="font-black text-slate-800 dark:text-white">Déposer un fichier Word (.docx)</p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">.docx uniquement</p>
                        </div>
