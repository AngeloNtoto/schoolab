...
/**
 * Modal d'upgrade - Version Compacte & Interactive
...
export default function UpgradeModal({ isOpen, onClose, featureName = 'cette fonctionnalité' }: UpgradeModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="relative bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-y-auto animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 border border-slate-200 dark:border-white/10"
      ...
      >
        ...
        {view === 'contact' ? (
          /* --- VUE CONTACT --- */
          <div className="p-8 lg:p-12 flex flex-col items-center justify-center md:min-h-[500px] text-center space-y-8 animate-in slide-in-from-right-4 fade-in duration-300">
            
            <div className={`p-6 rounded-full ${selectedPlan === 'PLUS' ? 'bg-gradient-to-br from-indigo-500/20 to-purple-500/20 text-indigo-500' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-500'}`}>
              {selectedPlan === 'PLUS' ? <Sparkles size={48} /> : <ShieldCheck size={48} />}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
              <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-3 p-5 bg-green-500 hover:bg-green-600 text-white rounded-2xl transition-all shadow-lg hover:shadow-green-500/30 group"
              >
                <Phone size={24} className="group-hover:rotate-12 transition-transform" />
                <div className="text-left">
                  <div className="text-[10px] font-black uppercase tracking-widest opacity-80">WhatsApp</div>
                  <div className="font-bold text-lg">+243 903 582 030</div>
                </div>
              </a>
              ...
              <a 
                ...
                className="flex items-center justify-center gap-3 p-5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-2xl transition-all border border-slate-200 dark:border-white/5"
              >
                <Mail size={24} className="text-slate-400" />
                 <div className="text-left">
                  ...
                  <div className="font-bold text-sm">NartrixSoft@gmail.com</div>
                </div>
              </a>
            </div>
          ...
          <div className="flex flex-col lg:flex-row h-full">
            ...
            <div className="p-6 text-center space-y-2 lg:hidden">
               <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full text-[10px] font-black uppercase tracking-widest">
                 <Sparkles size={10} />
                 Upgrade
               </div>
            ...
            <div className="flex-1 p-5 lg:p-8 pt-10 grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
              ...
              <div className={`rounded-2xl p-5 border flex flex-col relative z-0 transition-opacity duration-300 ${currentPlan === 'BASIC' || currentPlan === 'TRIAL' ? 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700' : 'bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                 <div className="space-y-2 mb-6 text-center">
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Découverte</p>
                 </div>
              ...
              <div className={`bg-white dark:bg-slate-900 rounded-2xl p-5 border-2 shadow-lg flex flex-col relative z-10 transition-colors ${currentPlan === 'PRO' ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-200 dark:border-white/10 hover:border-blue-200 dark:hover:border-blue-900/30'}`}>
                 <div className="space-y-2 mb-6 text-center">
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Établissement</p>
                 </div>
    ...
    <div className={`flex items-start gap-2.5 text-xs ${limited ? 'opacity-60' : ''}`}>
      <div className={`mt-0.5 flex-shrink-0`}>
        {cross ? (
          ...
          <CheckCircle size={14} className={highlight ? (color || 'text-indigo-500') : 'text-slate-300'} />
        )}
      </div>
      <span className={`${bold ? 'font-bold text-slate-800 dark:text-white' : 'font-medium text-slate-600 dark:text-slate-400'} ${cross ? 'text-slate-400 line-through decoration-slate-300' : ''}`}>
        {text}
      </span>
    </div>
  );
