            ...
            return sortedGroups.map(([key, groupSubjects]) => {
              ...
              return (
                <React.Fragment key={key}>
                  ...
                  {/* Matières de ce groupe — couleur réussite/échec évaluée PAR PÉRIODE */}
                  {groupSubjects.map((subject) => {
                    ...
                    return (
                      <tr key={subject.id}>
                        ...
                        <td className={`border border-black py-0.5 ${toneP1}`}>{p1 ?? ''}</td>
                        <td className={`border border-black py-0.5 ${toneP2}`}>{p2 ?? ''}</td>
                        {ex1 === null && firstSubject.max_exam1==0 ? (<td className="border bg-black border-black py-0.5"></td>) : (<td className={`border border-black py-0.5 ${toneEx1}`}>{ex1 ?? ''}</td>)}
                        <td className={`border border-black font-bold py-0.5 ${toneTot1}`}>{tot1 ?? ''}</td>
                        <td className={`border border-black py-0.5 ${toneP3}`}>{p3 ?? ''}</td>
                        <td className={`border border-black py-0.5 ${toneP4}`}>{p4 ?? ''}</td>
                        {ex2 === null && firstSubject.max_exam2==0 ? (<td className="border bg-black border-black py-0.5"></td>) : (<td className={`border border-black py-0.5 ${toneEx2}`}>{ex2 ?? ''}</td>)}
                        <td className={`border border-black font-bold py-0.5 ${toneTot2}`}>{tot2 ?? ''}</td>
                        <td className={`border border-black font-bold bg-slate-50 py-0.5 ${toneTG}`}>{tg ?? ''}</td>
