...
const GRID_CONFIG = {
  1: 'grid-cols-1 gap-0',
  2: 'grid-cols-1 grid-rows-2 gap-0',
  4: 'grid-cols-2 gap-3',
  6: 'grid-cols-2 gap-2',
};
...
export default function PrintLayout({
  ...
  const renderCoupon = (student: Student) => {
    ...
    const compactYearMode = period === 'YEAR' && couponsPerPage === 2;
        ...
        >
          <div className={`grid ${GRID_CONFIG[couponsPerPage]} h-full`}>
            {pageStudents.map((student) => (
              <div
                key={student.id}
                className={`coupon-wrapper ${COUPON_HEIGHT_CLASS[couponsPerPage]} overflow-hidden ${period === 'YEAR' && couponsPerPage === 2 ? 'flex' : ''}`}
              >
            ...
            {pageStudents.length < couponsPerPage &&
              Array(couponsPerPage - pageStudents.length)
                ...
                .map((_, i) => (
                  <div
                    key={`empty-${i}`}
                    className={`${COUPON_HEIGHT_CLASS[couponsPerPage]} ${period === 'YEAR' && couponsPerPage === 2 ? 'flex' : ''} border border-dashed border-slate-200 print:border-transparent`}
                  />
                ))}
          </div>
        </div>
