...
pub struct SyncData {
    // Correction du mismatch camelCase pour que l'API Cloud reçoive bien la clé "academicYears"
    #[serde(rename = "academicYears")]
...
fn collect_push_data(conn: &Connection) -> Result<SyncData, String> {
    ...
    let students_dirty: Vec<StudentPush> = conn.prepare("SELECT id, COALESCE(first_name, '') AS first_name, COALESCE(last_name, '') AS last_name, COALESCE(post_name, '') AS post_name, COALESCE(gender, '') AS gender, birth_date, COALESCE(birthplace, '') AS birthplace, COALESCE(conduite, '') AS conduite, COALESCE(conduite_p1, '') AS conduite_p1, COALESCE(conduite_p2, '') AS conduite_p2, COALESCE(conduite_p3, '') AS conduite_p3, COALESCE(conduite_p4, '') AS conduite_p4, is_abandoned, COALESCE(abandon_reason, '') AS abandon_reason, class_id, server_id, last_modified_at FROM students WHERE is_dirty = 1")
...
fn process_push_response(conn: &Connection, response: PushResponse) -> Result<(), String> {
    if response.success {
        ...
        // Enregistre l'heure de la dernière synchronisation push réussie
...
fn process_pull_data(mut conn: Connection, data: PullData) -> Result<i32, String> {
    ...
    // Enregistre l'heure du dernier pull réussi
    let now = chrono::Utc::now().to_rfc3339();
    let _ = tx.execute(
        "INSERT OR REPLACE INTO settings (key, value) VALUES ('last_sync_time', ?)",
        params![now],
...
pub async fn sync_push(app_handle: tauri::AppHandle) -> Result<SyncResult, String> {
    ...
    if push_total == 0 {
        // Enregistre l'heure de la synchronisation même si rien n'était à envoyer
