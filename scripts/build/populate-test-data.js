...
async function populateTestData() {
    const dbPath = (0, path_1.join)(electron_1.app.getPath('userData'), 'school.db');
    ...
    `).get(academicYear.id);
      ...
      `).get(subject.name, testClass.id);
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(subject.name, subject.code, 10, // max_p1
                10, // max_p2
                20, // max_exam1
                10, // max_p3
                10, // max_p4
        ...
        for (let i = 0; i < 100; i++) {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(firstName, lastName, '', // post_name
                gender, birthdate, birthplace, testClass.id);
